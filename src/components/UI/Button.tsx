...
const buttonStyles = {
  base: cn('px-3.5 py-3', 'border border-white-dirty rounded-md', 'active:scale-95'),
...
export default function Button({to, text, className, onClick}: {text: string; to?: string; className?: string; onClick?: () => void}) {
  ...
  const content = (
    <>
      ...
      <P className="relative z-10 transition-all duration-500 group-hover/button:text-black uppercase">{text}</P>{' '}
    </>
  )
